...
| 58 | **Sprint 4 — Endgame Conversion Repair:** Diagnostic: AB_D4 0/5 Tier A conversions vs AZ-best 4/5 (3–23 ply). Depth sweep D4–D10 all DRAW → classified OBJECTIVE. Implemented dedicated conversion mode in `ab_search.cpp`: `detect_conversion()` trigger, `evaluate_conversion()` lexicographic eval (mobility×50 > check×30 > approach > proximity > confine), check extension (+2 ply), low-mobility extension (+1 ply), twofold rep penalty (−500), auto depth boost D4→D8, stalemate bonus (was penalty). Fixed `terminal_info` priority in `rules.cpp` (legal moves checked before repetition/ply limit). **Result**: AB squeezes General to 0 moves repeatedly but game ends by threefold repetition @ ply 393. **Gate FAILED, hard stop rule triggered → AB demoted to appendix**. Negative finding: heuristic eval cannot encode tactical mating patterns in asymmetric endgames; AZ neural net required. |
