...
def main():
    ...
    parser.add_argument("--kind", choices=("cpu", "gpu", "debug"), required=True)
    ...
    with ledger.with_suffix(".lock").open("a") as lock:
        ...
        data = json.loads(ledger.read_text()) if ledger.exists() else {"gpu_seconds_limit": 28800,
            "cpu_core_seconds_limit": 230400, "jobs": []}
        ...
        atomic_json(ledger, data)
        submit = ["sbatch", "--parsable", "--account=gratis", "--partition="+("gpu" if gpu else "epyc2"),
            "--qos="+("job_debug" if args.kind == "debug" else "job_gratis"), "--nodes=1", "--ntasks=1",
